...
<link rel="stylesheet" href="<?php echo SRC_TPL_DIR;?>/css/55.css">
<style>
  .tip{font:bold 14px/2.5 'microsoft yahei';}
  .qq_kf{margin: 10px 0 20px;}
  .qq_kf li{padding-left: 12px; display: flex; align-items: center; gap: 5px;}
  .qq_kf li img{vertical-align: middle; flex-shrink: 0;}
  .clearfix:before,
.clearfix:after {
  content: "";
  display: table;
...
<script type="text/javascript" src="<?php echo SRC_TPL_DIR?>/images/jquery-1.7.min.js"></script>
    ...
    <div class="wrapper">
        <div class="sub_obj_a cd-headline letters scale">
            <span class="cd-words-wrapper" style="width: 365px;">
                 <b class="is-visible"><i class="in">现</i><i class="in">在</i><i class="in">加</i><i class="in">入</i><i class="in">&nbsp;</i><i class="in">-</i><i class="in">&nbsp;</i><i class="in">梦</i><i class="in">想</i><i class="in">之</i><i class="in">旅</i><i class="in">从</i><i class="in">此</i><i class="in">开</i><i class="in">始</i></b>
...
<form action="<?php echo url("index.post_register");?>" method="POST" class="form-horizontal" id="form_b" >
  ...
  <table width="960" border="0" align="center" valign="middle" cellpadding="0" cellspacing="0" class="register" style="margin:auto;">
    ...
    <tr>
      <td class="tip">用户名信息：请正确填写用户名信息，提供本人"Telegram"，遇到产品下线，报毒等问题及时与你联系</td>
    </tr>
    <tr>
    ...
    <tr>
      <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="input_1" style="margin-bottom: 15px">
          ...
          <tr>
            <td width="180"><font color="#FF0000">*</font>Telegram</td>
            <td><input type="text" name="qq" id="qq" class="input_text" placeholder="@英文"></td>
          </tr>
          <tr>
      ...
      <tr class="aff_r">
        <td class="tip">钱包信息（请正确填写，注册后无法修改）</td>
      </tr>
      <tr class="aff_r">
        <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="input_1" style="margin-bottom: 15px">
            <tbody><tr>
              <td width="180"><font color="#FF0000">*</font>钱包类型</td>
            ...
            <tr>
              <td><font color="#FF0000">*</font>钱包地址</td>
        ...
        <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="input_1">
            <tbody><tr>
              ...
              <td><ul class="qq_kf clearfix">
                                   <?php if(!empty($serviceuser) && is_array($serviceuser)){ foreach($serviceuser AS $s){?>
                   <li>
                     <input style="transform: scale(1.5,1.5);" name="serviceid" type="radio" value="<?php echo $s['uid']?>"   />
                     <span><?php echo $s['contact']?></span> <a target="_blank" href="https://t.me/<?php echo ltrim($s['qq'], '@');?>" style="padding-left:1px;"><img src="<?php echo SRC_TPL_DIR;?>/images/telegram.png" alt="点击这里给我发消息" border="0" align="absmiddle" title="点击这里给我发消息" width="18" height="18" /></a></li>
                  <?php } } else { ?>
                  <li style="color:#999;">暂无客服</li>
                  <?php } ?>
                                  </ul></td>
            </tr>
          </tbody></table></td>
      ...
      <tr class="adv_r" style="display: none;">
        <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="input_1">
            <tbody><tr>
              ...
              <td><ul class="qq_kf clearfix">
                                  <?php if(!empty($commercialuser) && is_array($commercialuser)){ foreach($commercialuser AS $s){?>
                   <li>
                    <input style="transform: scale(1.5,1.5);" name="serviceid" type="radio" value="<?php echo $s['uid']?>"   />
                     <span><?php echo $s['contact']?></span> <a target="_blank" href="https://t.me/<?php echo ltrim($s['qq'], '@');?>" style="padding-left:1px;"><img src="<?php echo SRC_TPL_DIR;?>/images/telegram.png" alt="点击这里给我发消息" border="0" align="absmiddle" title="点击这里给我发消息" width="18" height="18" /></a></span></li>
                  <?php } } else { ?>
                  <li style="color:#999;">暂无商务</li>
                  <?php } ?>
                                  </ul></td>
            </tr>
          </tbody></table></td>
    ...
    <tr>
      <td><div style="background-color:#FAFAFA;border:1px solid #CCCCCC;height:220px; overflow:auto;padding:5px;text-align:left;width:963px; margin-left:2px"> 一、合作协议的确认和接纳<br>
          海森广告（）所提供的合作项目的所有权和运作权归海森广告所有。您通过完成注册程序并点击选择“同意以下服务条款，提交注册信息”，表示您与海森广告网站签订本合作协议（下称“本合作协议”）并接受所有相关条款的约束，以及表示您成为海森广告的成员（“联盟成员”）。
          <p></p>
          <p>二、 合作内容及费用说明<br>
            海森广告成员注册成功后，即可登录系统，获得相关产品代码。联盟成员在其网站页面中嵌入其从系统中获得的产品代码后，该网站的访问者可以获得海森广告提供的产品及/或相关服务。<br>
            海森广告根据各种产品的不同计费方式统计带有联盟成员特征代码的产品流量，并按此流量向海森广告成员支付相应合作费用。其各类产品收益模式及分成比例请登录后仔细阅读相关说明，合作费用支付以海森广告（）网站上发布的标准为准，除非海森广告与联盟成员间有另行约定并就此另行达成书面协议的。海森广告有权根据联盟策略随时更改产品的收益模式、分成比例及结算策略，您在此同意始终遵守。</p>
          <p>关于违规及作弊<br>
            加海森广告的成员，必须自觉遵守本合作协议，如发现任何违规或作弊行为，海森广告将停止付费并取消其联盟成员资格，同时保留进一步追究法律责任的权利。</p>
          <p>联盟代码能且只能投放在联盟成员提交的域名下，否则被视为违规投放。若要在多个网站或域名下投放联盟代码，须在注册成功后，在“会员管理中心”中登记所有投放海森广告代码的域名。</p>
          <p>违规或作弊行为包含但不限于：<br>
            1. 任何形式的分拆代码：<br>
          ...
          <p>三、联盟成员的注册规则<br>
            ...
            D、如联盟成员违反上述规定，应自行承担相应法律后果；海森广告有权在必要时采取相应措施以维护相关合法权益。<br>
            2.<br>
            联盟成员的帐号、密码和安全性：您一旦注册成功并经海森广告审核通过，成为海森广告的合法成员，将得到一个密码和用户名。联盟成员将对用户名和密码安全负全部责任，且联盟成员对以其用户名在海森广告进行的所有行为承担全部责任。联盟成员必须保管好自己的用户名和密码，谨防被盗或泄露；如因保管不善导致帐号和密码被盗或泄露，联盟成员自行承担相应后果。如果联盟成员在发现有任何非法使用联盟成员帐号或安全漏洞的情况，可以立即通告海森广告以协助处理。<br>
            3.<br>
            提供详尽、准确的个人资料，并不断更新注册资料，符合及时、详尽、准确的要求。真实的联盟成员资料将作为海森广告提供服务的依据和联盟成员获得法律保障的前提。若联盟成员提供的资料包含有不正确的信息，海森广告保留停止向该联盟成员付费的权利，因虚假资料造成的任何损失，与海森广告无关，海森广告不负任何责任。</p>
          <p>四、 隐私保护政策<br>
            每当联盟成员提供给海森广告个人信息时，海森广告会采取合理的步骤保护联盟成员的个人信息，海森广告也会采取合理的安全手段保护已存储的个人信息。由于网上技术发展的突飞猛进，海森广告会随时更新并公布海森广告的信息保密制度，除非以下三种情况外，海森广告不会在未经合法联盟成员授权时，公开、编辑或透露联盟成员资料及其它保存在海森广告中的保密性内容：<br>
            1) 根据有关法律规定要求；<br>
            2) 有关权力机关要求；<br>
            3)<br>
            联盟成员授权（包括，如果联盟成员要求海森广告提供特定服务时，海森广告则需要把联盟成员的姓名和地址提供给与此相关联的第三方如邮政服务单位）。</p>
          <p>五、 暂停与终止合作<br>
            海森广告将根据联盟成员合作效果保留暂停或终止合作的权利。<br>
            任何经海森广告确认已违反了合作协议的联盟成员，海森广告有权决定是否给予其暂停或终止合作的处理，对屡犯者将立即给予暂停合作或终止合作的处理。<br>
            海森广告保留其单方暂停或终止本合作协议的权利。但若海森广告单方决定终止的，应提前一个月通知。通知做出的形式依据本合作协议第七条的规定。</p>
          <p>六、 风险分担<br>
            联盟成员使用本网站将承担一定风险：海森广告将不承担由于联盟成员自身过错、技术或其它不可控原因导致网站页面信息或其它方面的错误而给联盟成员造成的损失。</p>
          <p>七、 通知<br>
            所有发给联盟成员的通知都将通过网站页面的公告或电子邮件传送。协议条款的修改、服务变更、或其它重要事件的通知都会以此形式进行。</p>
          <p>八、 协议条款和资费标准的修改<br>
            海森广告有权在必要时修改本合作协议条款的内容和合作资费标准，且该修改以符合国家法律法规的规定，并不侵害联盟成员的合法权益为必要前提。海森广告合作协议条款一旦发生变动，海森广告将在网页上公布修改内容。修改后的合作协议一旦在网页上公布即有效代替原来的合作协议。您可随时造访查阅最新版合作协议。联盟成员若不同意合作协议改动的内容，可以主动退出海森广告；若海森广告没有收到联盟成员书面通知，明确表示其退出海森广告的，则视为该联盟成员选择继续享用海森广告服务，已接受该协议条款的变动并受变动后协议条款的约束。</p>
          <p>九、 法律适用<br>
            本合作协议条款应符合中华人民共和国法律法规的规定，联盟成员和海森广告一致同意接受中华人民共和国法律的管辖。当本合作协议条款与中华人民共和国法律相抵触时，则这些条款将完全按法律规定重新解释或重新修订，而其它条款则依旧对海森广告及联盟成员产生法律效力。</p>
          <p>十、 争议解决<br>
            与本合作协议有关的一切争议，双方应通过协商方式友好解决；如协商未果，应将争议提交北京仲裁委员会进行仲裁，该仲裁结果是终局的并对双方均有约束力。</p>
          <p>十一、海森广告欢迎联盟成员对网站服务条款给予评论或提出质疑。<br>
            海森广告 将根据网站发展的需求和技术的发展不断完善本合作协议。请将与本合作协议有关的所有评论或疑问发往 a@<?php echo SITE_AFTER;?>。<br>
            警告： <br>
            对任何违反国家法律和海森广告网站相应管理规定且侵害了海森广告网站合法权益的行为，海森广告将保留追究其法律责任的权利。</p>
        </div></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
...
// 添加自定义 Telegram 验证方法（失焦后校验）
$.validator.addMethod("telegram", function(value, element) {
    var trimmedValue = (value || '').trim();
    // 如果值为空，返回 false（让 required 规则处理）
    if (trimmedValue === '') {
        return false;
    }
    // 开头必须是 @，支持英文字母、数字和下划线，长度不小于2个字符
    return trimmedValue.length >= 2 && /^@[a-zA-Z0-9_]+$/.test(trimmedValue);
}, "@英文");
