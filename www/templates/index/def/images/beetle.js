...
jQuery(document).ready(function ($) {
    ...
    function googleMap() {
        ...
        if (typeof google === 'undefined' || !google.maps) {
            console.warn('Google Maps API 未加载，跳过地图初始化');
            return;
        }
        ...
        try {
            ...
            */
            ...
            console.warn('Google Maps 初始化失败:', error);
        }
    }

    if ($('.map').length) {
        // 检查 Google Maps API 是否已加载，如果未加载则跳过
        if (typeof google !== 'undefined' && google.maps) {
            googleMap();
        } else {
            console.warn('页面包含地图元素，但 Google Maps API 未加载');
        }
    }
